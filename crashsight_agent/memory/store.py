...
class MemoryStore:
    ...
    def cleanup_episodes(self, max_age_days: int = 90, max_count: int = 5000):
        ...
        if total_deleted > 0:
            logger.info(f'遗忘: 清理了 {total_deleted} 条过期记忆 (过期{deleted_age} + 溢出{deleted_overflow})')
    ...
    def _try_extract_skill(self, query: str, intent: str, project_id: str, version: str):
        ...
        if count >= 3:
            ...
            if existing:
                ...
                logger.info(f'新 Skill 提炼: {pattern} (出现{count}次)')
    ...
    def _check_failure_pattern(self, intent: str, project_id: str):
        ...
        logger.warning(f'三层联动: {pattern_desc} 连续失败3次，自动提炼规则')
    ...
    def add_rule(self, rule_text: str, category: str = 'general', confidence: float = 0.7,
        ...
        conn.commit()
        conn.close()
        logger.info(f'新规则: [{category}] {rule_text}')
